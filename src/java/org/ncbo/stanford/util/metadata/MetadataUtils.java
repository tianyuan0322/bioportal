...
/**
 * Super class of all ...MetadataUtil classes 
 * defining some commonly used constants and
...
public class MetadataUtils {
	...
	protected static void setPropertyValue(OWLModel owlModel, RDFResource owlInd, String propName, Object value) throws Exception {
	...
	protected static <T> T getPropertyValue(OWLModel owlModel, RDFResource owlInd, String propName, Class<T> type) throws Exception {
		OWLProperty prop = owlModel.getOWLProperty(propName);
		if (prop == null) {
			throw new Exception("Metadata ontology does not contain property " + propName);
		}
...
//						String stringValue = lit.getString();
//						SimpleDateFormat.
//						val = new Date(Date.parse());
	...
	protected static <T> List<T> getPropertyValues(OWLModel owlModel, RDFResource owlInd, String propName, Class<T> type) throws Exception {
		OWLProperty prop = owlModel.getOWLProperty(propName);
		if (prop == null) {
			throw new Exception("Metadata ontology does not contain property " + propName);
		}
	...
	protected static Integer getId(OWLModel owlModel,
			RDFResource owlInd) throws Exception {
		Collection<Integer> propVals = getPropertyValues(owlModel, owlInd, PROPERTY_ID, Integer.class);
		if (propVals == null || propVals.isEmpty()) {
			return null;
		} else {
			if (propVals.size() == 1) {
	...
	protected static int getNextAvailableIdForClass(OWLClass cls) throws Exception {
