...
public class DiffFilePathHandlerImpl implements DiffFilePathHandler {
	...
	/**
	 * Returns a directory path for the directory that has all the diffs for the
	 * given ontology If the directory does not exist, returns null
	 * 
	 * @param ontologyId
	 * @return
	 */
	...
	/**
	 * Checks if a directory for the diffs between two versions exists. If
	 * createDir is true, creates the directory; if it is false, returns null if
	 * directory does not exist, directory name otherwise
	 * 
	 * @param ontologyId
	...
	public String getDiffDirNameForOntologyVersions(Integer ontologyId,
			...
			boolean createDir) {
		String dirPath = getDirPathForOntology(ontologyId) + "/"
				+ ontologyVersionIdOld + SEPARATOR_IN_DIR_NAME
				+ ontologyVersionIdNew;

		File diffDirectory = new File(dirPath);

		if (createDir)
			diffDirectory.mkdirs();
		else // we do not want to create a diff directory, we were trying to
		// get the diff file
	...
	/**
	 * Creates a file name given a directory id and the two version ids. Does
	 * not add the extension to the file
	 * 
	 * @param diffDirName
	...
	private String getDirPathForOntology(Integer ontologyId) {
		return MessageUtils.getMessage("bioportal.diff.filepath") + "/"
				+ ontologyId;
	}
