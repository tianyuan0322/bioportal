 ...
 * work with {@link org.acegisecurity.userdetails.UserDetails} objects. The
 * class is designed to respond to {@link UsernamePasswordAuthenticationToken}
 * authentication requests. This is a custom implementation that allows
 * additional application-based authentication.
 * 
 * <p>
 ...
 * incorrect password, the {@link UserDetailsService} will be queried to confirm
 * the most up-to-date password was used for comparison.
 * </p>
 * 
 * @author Michael Dorf
 */
public abstract class AbstractRESTfulUserDetailsAuthenticationProvider
	...
	protected boolean hideApplicationNotFoundExceptions = true;
	 ...
	 * Authentication#getCredentials()} with a {@link UserDetails#getPassword()}.
	...
	protected abstract void additionalAuthenticationChecks(
			UserDetails userDetails, ApplicationBean application,
			UsernamePasswordAuthenticationToken authentication)
			throws AuthenticationException;

	public final void afterPropertiesSet() throws Exception {
		Assert.notNull(this.userApplicationCache,
				"A user/application cache must be set");
	...
	public Authentication authenticate(Authentication authentication)
		...
		String applicationId = ((ApplicationAuthenticationDetails) authentication
				.getDetails()).getApplicationId();
		applicationId = (applicationId == null) ? "NONE_PROVIDED"
				: applicationId;

		boolean cacheWasUsed = true;
		UserDetails user = this.userApplicationCache.getUserFromCache(username);
		ApplicationBean application = this.userApplicationCache
				.getApplicationFromCache(applicationId);
		...
		if (user == null || application == null) {
			...
			try {
				application = retrieveApplication(applicationId,
						(UsernamePasswordAuthenticationToken) authentication);
			} catch (ApplicationNotFoundException notFound) {
				if (hideApplicationNotFoundExceptions) {
					throw new BadCredentialsException(
							messages
									.getMessage(
											"AbstractUserDetailsAuthenticationProvider.badCredentials",
			...
			Assert
					.notNull(application,
							"retrieveApplication returned null - a violation of the interface contract");
		}
		...
		try {
			additionalAuthenticationChecks(user, application,
					(UsernamePasswordAuthenticationToken) authentication);
		...
		if (!cacheWasUsed) {
			this.userApplicationCache.putUserInCache(user);
			this.userApplicationCache.putApplicationInCache(application);
		}
	 ...
	 * from an implementation-specific location, with the option of throwing an
	 ...
	 * means subsequent requests that rely on the cache will need to still have
	 * their credentials validated, even if the correctness of credentials was
	 * assured by subclasses adopting a binding-based strategy in this method.
	 ...
	 * of authenticating a request, as no <code>UserDetails</code> will ever
	 * be cached) or ensure subclasses implement
	 ...
	 *            need to perform a binding-based retrieval of the
	 *            <code>UserDetails</code>
	 * 
	...
	protected abstract ApplicationBean retrieveApplication(
	 ...
	 * throws a <code>BadCredentialsException</code> if a username is not
	 * found or the password is incorrect. Setting this property to
	 * <code>false</code> will cause <code>UsernameNotFoundException</code>s
	 * to be thrown instead for the former. Note this is considered less secure
	 * than throwing <code>BadCredentialsException</code> for both exceptions.
	 * 
	 * @param hideUserNotFoundExceptions
	 *            set to <code>false</code> if you wish
	 *            <code>UsernameNotFoundException</code>s to be thrown
	...
	/**
	 * @return the hideApplicationNotFoundExceptions
	 */
	public boolean isHideApplicationNotFoundExceptions() {
		return hideApplicationNotFoundExceptions;
	}

	/**
	 * @param hideApplicationNotFoundExceptions
	 *            the hideApplicationNotFoundExceptions to set
	 */
	public void setHideApplicationNotFoundExceptions(
			boolean hideApplicationNotFoundExceptions) {
		this.hideApplicationNotFoundExceptions = hideApplicationNotFoundExceptions;
	}

	/**
	 * @return the userApplicationCache
	 */
	public UserApplicationCache getUserApplicationCache() {
		return userApplicationCache;
	}

	/**
	 * @param userApplicationCache
	 *            the userApplicationCache to set
	 */
	public void setUserApplicationCache(
			UserApplicationCache userApplicationCache) {
		this.userApplicationCache = userApplicationCache;
	}
}
