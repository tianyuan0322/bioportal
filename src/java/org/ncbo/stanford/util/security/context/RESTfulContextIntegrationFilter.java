 ...
 */
public class RESTfulContextIntegrationFilter implements InitializingBean, Filter {
    //~ Static fields/initializers =====================================================================================

    protected static final Log logger = LogFactory.getLog(RESTfulContextIntegrationFilter.class);
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException,
        ...
        String sessionId = request.getParameter(RequestParamConstants.PARAM_SESSIONID);
        ...
        logger.debug("session not null: " + httpSessionExistedAtStartOfRequest);
...
//        httpSession = null;
        ...
        OnRedirectUpdateSessionResponseWrapper responseWrapper =
                new OnRedirectUpdateSessionResponseWrapper(response, session,
        ...
        try {
            ...
            if (!responseWrapper.isSessionUpdateDone()) {
              storeSecurityContextInSession(contextAfterChainExecution, session,
                      httpSessionExistedAtStartOfRequest, contextHashBeforeChainExecution);
            }
     ...
     *
     * @param session the session obtained from the request.
     */
    private SecurityContext readSecurityContextFromSession(RESTfulSession session) {
        if (session == null) {
            if (logger.isDebugEnabled()) {
        ...
        Object contextFromSessionObject = session.getAttribute(ACEGI_SECURITY_CONTEXT_KEY);
    ...
    private void storeSecurityContextInSession(SecurityContext securityContext,
        ...
        if (session != null && securityContext.hashCode() != contextHashBeforeChainExecution) {
            session.setAttribute(ACEGI_SECURITY_CONTEXT_KEY, securityContext);

            if (logger.isDebugEnabled()) {
                logger.debug("SecurityContext stored to Session: '" + securityContext + "'");
            }
        }
    }
    ...
    private class OnRedirectUpdateSessionResponseWrapper extends HttpServletResponseWrapper {
         ...
         * @see RESTfulContextIntegrationFilter#storeSecurityContextInSession(SecurityContext, ServletRequest, boolean, int)
         */
