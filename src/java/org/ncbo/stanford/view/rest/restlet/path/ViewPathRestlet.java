...
public class ViewPathRestlet extends AbstractBaseRestlet {

	@SuppressWarnings("unused")
	private static final Log log = LogFactory.getLog(ViewPathRestlet.class);
	...
	private void findPathFromRoot(Request request, Response resp) {
		...
		try {
			Integer ontViewId = Integer.parseInt(ontologyViewVersionId);
			if (target
					.equalsIgnoreCase(RequestParamConstants.PARAM_ROOT_CONCEPT)) {
				concept = conceptService.findPathFromRootInView(ontViewId, source, light);
			} else {
