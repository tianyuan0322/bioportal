...
public class PropertySearchRestlet extends Restlet {

	private static final Log log = LogFactory.getLog(PropertySearchRestlet.class);
	 ...
	 * @param resp
	 */
	private void searchProperty(Request request, Response resp) {
		...
		try {
			...
			if(!ontologies.equalsIgnoreCase(RequestParamConstants.PARAM_ALL_ONTOLOGIES)){
			...
			concepts = conceptService.findConceptPropertyContains(ontologyIds, query);
		...
		getXmlSerializationService().generateXMLResponse (request, resp, concepts);
	}
