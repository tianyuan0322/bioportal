...
public class ConceptServiceTest extends AbstractBioPortalTest {
	...
	@Autowired
	OntologyLoadManagerProtegeImpl ontologyManager;

	@Test
	public void testObsoleteConceptRetrieval() throws Exception {
		OntologyBean ncit = ontologyService.findOntologyOrView(45400);

		KnowledgeBase kb = ontologyManager.getKnowledgeBase(ncit);

		OWLClass obsolete_concept = ((OWLModel) kb).getOWLNamedClass("Retired_Concepts");

//		RDFResource obsoleteProperty = ((OWLModel) kb).getC.getRDFProperty(name);
//		Collection<?> classesWithObsoleteProperty = ((OWLModel) kb).getMatchingResources(property, matchString, maxMatches);

		long start = System.currentTimeMillis();
		Collection<?> obsoleteConcepts = obsolete_concept.getSubclasses(false);
		System.out.println("Get obsolete subclasses ("+ obsoleteConcepts.size() + "): " + (System.currentTimeMillis() - start) + "ms");

		OWLClass obsoleteConceptToCompare = ((OWLModel) kb).getOWLNamedClass("_1-Aminocyclopentane");

		System.out.println("Class is obsolete: " + obsoleteConcepts.contains(obsoleteConceptToCompare));


		// Get all classes with property X and the given value
		RDFProperty deprecatedProperty = ((OWLModel) kb).getRDFProperty("http://deprecated.classes.com");
		((OWLModel) kb).getMatchingResources(deprecatedProperty, "value", Integer.MAX_VALUE);

		// Get all classes with property X and any value
		((OWLModel) kb).getHeadFrameStore().getFramesWithAnyDirectOwnSlotValue(deprecatedProperty);
	}
